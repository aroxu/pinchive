...
def save_cookies_text(cred_id: int, raw: str) -> int:
    """Persist pasted cookies for a credential id. Returns cookies written."""
    return save_cookies_text_to(cookies_path(cred_id), raw)


def save_cookies_text_to(path: Path, raw: str) -> int:
    """Persist pasted cookies (Netscape or JSON) as a Netscape file at `path`."""
    raw = raw.strip()
    if not raw:
